...
//					Return							= _Internet.PostData(URL, string.Empty);
	...
	//								FindLinkStop	= Temp.IndexOf(" ", FindLinkStart) - 1;
								...
								if(Temp.IndexOf("'", FindLinkStart + 4) < FindLinkStop)
									FindLinkStop	= Temp.IndexOf("'", FindLinkStart + 4);

								if(Temp.IndexOf(Convert.ToChar(34).ToString(), FindLinkStart + 4) < FindLinkStop)
								...
								if( FindLinkStop > 0 )
								{
									...
									if( Link.ToUpper().IndexOf("SCRIPT") > -1 )
									{
										...
										if( FindLinkStart > -1 )
										{
											...
											if(Temp.IndexOf("'", FindLinkStart + 1) < PageLength)
												FindLinkStop		= Temp.IndexOf("'", FindLinkStart + 1);

											if(Temp.IndexOf(Convert.ToChar(34).ToString(), FindLinkStart + 1) < FindLinkStop)
											{
//												FindLinkStop		= Temp.IndexOf(Convert.ToChar(34).ToString(), FindLinkStart + 1) + 50;
									...
									//if( Link.ToUpper().StartsWith("HTTP") )
									if( Link.ToUpper().StartsWith("HTTP") && !Link.ToUpper().StartsWith(_DomainName) )
									{
										// should we save to the database
										if( _SaveData == true )
										{
